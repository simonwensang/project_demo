...
public enum McResultCode {
	...
	LOGIN_PARAM_INVALID("-404",  "%s"),

    EXTERNAL_SYSTEM_ERROR("1000",  "调用外部系统[%s]出错"),
	DIGITAL_SEARCH_FIELD("1001", "商品查询出错，%s"),
	TOO_MANY_REQUESTS("1002", "请求过多"),
    SYS_BUSY("1003", "服务器繁忙,请稍后重试"),

	REGISTER_ERROR("1004","%s和%s已经注册"),
	COMMAND_ERROR("1005", "%s命令格式错误"),
	REGISTER_MOBILE_ERROR("1006","%s已经注册"),
	REGISTER_CAPTCHA_ERROR("1007", "验证码%s分钟内只能发送一次"),
	SHORT_MESSAGE_ERROR("1008", "手机号为%s的短信发送失败"),
	...
	REMAINDER_NOT_ENOUGH("10046", "库存不足"),
	BIND_STATUS_ERROR("10047", "状态错误，不是%s"),

	BATCH_CREATE_IOT_MACHINE_ERROR("10048","批量申请设备失败！%s"),
	FIELD_VALUE_MUST_EQUAl_THAN("10049", "%s必须等于%s"),
	COMMON_PARAM_ERROR("10050", " %s错误！"),
	//----------------------------------------
	// api调用异常
	// 区间：10051-10099
	//----------------------------------------
	...
	COUPON_PER_ORDER_LIMIT("10111","该促销每订单限制最多使用%s张"),
	PROMOTION_STATUS_INVALID("10208","促销状态非法，促销已过期或未开始"),
	PROMOTION_TIME_NOT_IN_ORDER_TIME("10209", "下单时间不在促销有效时间内"),
...
//    COUPON_ISSUED_TIME_HAS_EXPIRED("10305", "优惠券发券时间已过"),
	...
	COUPON_OUT_OF_COMMISSION("10319", "券的状态不在[报名中]或[进行中]，不能使用！"),
	COUPON_SETTING_ERROR("10320", "券设置错误，原因[%s]"),
	COUPON_PACKAGE_SETTING_ERROR("10321", "券包设置错误，原因[%s]"),
	COUPON_NOT_TAKE("10322", "%s不可领取"),
	//----------------------------------------
	// 促销异常
	// 区间：10400-10499
	//----------------------------------------
	...
	ITEM_PROMOTION_GROUP_BY_DATE_ERR("10518", "当前时间不在组团时间段内"),
	ITEM_PROMOTION_GROUP_BY_ONCE("10519", "不能重复参与组团"),
	ITEM_PROMOTION_TIME_ERR("10520", "当前时间不在支付尾款时间段内"),
	...
	PROMOTION_PRICE_MORE_SKU_PRICE("10833", "爆款价不得高于%s价的百分之%s"),
	COUPON_COUNT_MORE_ISSUED_AMOUNT("10834", "优惠券可核销数量超过发券总数量！发券总数量为:%s"),
	LARGEST_SHOP_NUMBER("10835", "单次新增下发商户不能超过200家！"),
	REQUIRE_BOOKING_ERROR("10836", "预定时间已开始，请勿更新数据！%s"),
	NO_ISSUED_BRAND("10837","%s没有下发给工厂！"),
	...
	SKU_DATETIME_INTERSECTION("10901", "提报的商品在其他同期促销中已经是爆款，%s"),
	...
	SKU_PRICE_MORE_SALE_PRICE("10907", "提报商品的价格必须小于%s价！ skuId：%s"),
	...
	SKU_SUBNUM_ERROR("10914","同一时间段爆款不能超过 %d!"),
	SKU_SUBPERNUM_ERROR("10915","同一个促销爆款不能超过 %d!"),
	SKU_STOCK_MORE_NUMBER("10916", "提报库存必须大于等于成团数量！ skuId：%s"),
	//----------------------------------------
	// 51零元抢购活动异常
	// 区间：11000-11099
	//----------------------------------------
	...
	COUPON_PACKAGE_LEAD_END_ERROR("1019", "%s券包已领完"),
	...
	COUPON_STOCK_NOT_ENOUGH("11237", "券数量不足，请先至相应模块增加券数量"),
	REDUCE_SCORE_ERROR("11238", "扣减积分失败"),
	ACTIVITY_NOT_SUPPORT_SCORE("11239", "本次活动不支持积分兑换抽奖机会"),
	ACTIVITY_MAX_DRAW_COUNT("11240", "本次活动最多可兑换%s次抽奖机会"),
	ACTIVITY_DRAW_COUNT_DELAY("11241", "积分扣减成功,抽奖机会稍后发放"),
    ...
    McResultCode(String code, String message) {
        this.code = code;
